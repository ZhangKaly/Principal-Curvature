...
for i in tqdm(range(num_eval)):
    a, b = compute_curvature(torus_cloud, np.expand_dims(torus_cloud[i], axis=0), epsilon_PCA = 0.2, tau_ratio = 4)
    curvature.append(b)

v = np.array(curvature).T
savetxt('curvature_ratio_4_1000_2.csv', v, delimiter=',')
savetxt('torus_cloud_ratio_4_1000_2.csv', torus_cloud, delimiter=',')
...
plt.axis('off')
ax.set_aspect('equal')
plt.savefig("torus_ratio_4_1000_2.png", dpi = 300)
...
plt.axis('off')
ax.set_aspect('equal')
plt.savefig("torus_birdview_ratio_4_1000_2.png", dpi = 300)