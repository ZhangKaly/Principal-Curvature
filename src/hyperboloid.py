...
if __name__ == "__main__":
    ...
    # print("hyperboloid generation started.")
    # hyperboloid = generate_hyperboloid_cloud(1, 1, 0.5)
    # print("hyperboloid generated.")
    ...
    # hyper_cutted_index = []
    # print("hyperboloid cut started.")

    # for i in range(len(hyperboloid)):
    #     if abs(hyperboloid[i, 2]) <= 1.6:
    #         hyper_cutted_index.append(i)
    #     print(f"{i}th point, in total {len(hyper_cutted_index)} cutted.")
    # print("hyperboloid cut generated.")

    # # fig = plt.figure()
    # # ax = fig.add_subplot(111, projection='3d')
    # # ax.scatter(hyperboloid[hyper_cutted_index, 0], hyperboloid[hyper_cutted_index, 1], hyperboloid[hyper_cutted_index, 2], s=5)
    # # ax.set_title("Generated Point Cloud")
    # # ax.view_init(0, 0)
    # # plt.savefig('hyperboloid_cutted.png')
    # # plt.show()


    # np.savetxt('hyperboloid_cloud_ratio_4.csv', hyperboloid, delimiter=',')
    # np.savetxt('hyperboloid_cloud_ratio_4_cutted_index.csv', hyper_cutted_index, delimiter=',')
    ...
    #     a, b = compute_curvature_v2(hyperboloid, np.expand_dims(hyperboloid[i], axis=0), epsilon_PCA = 0.1, tau_ratio = 4)
    ...
    assert eval_id_end <= len(hyper_cutted_index)
    print(f"evaluating points {eval_id_start}~{eval_id_end}")

    def compute_curvature_for_point(i):
        ## TODO REMEMBER THE i IS THE INDEX IN hyper_cutted_index!!!
        j = i + eval_id_start
        # print(f"point {j} started.")
        try:
            res = compute_curvature_v2(hyperboloid, np.expand_dims(hyperboloid[hyper_cutted_index[j]], axis=0), epsilon_PCA = 0.1, tau_ratio = 4)[1]
        except ValueError:
            print(f"Warning for point {hyper_cutted_index[j]}: ValueError: matmul: Input operand 1 has a mismatch in its core dimension 0, with gufunc signature (n?,k),(k,m?)->(n?,m?) (size 2 is different from 3)")
            return np.nan
    ...
    np.savetxt(f'curvature_hyperboloid_ratio_4_from_{eval_id_start}_to_{eval_id_end}.csv', v, delimiter=',')
