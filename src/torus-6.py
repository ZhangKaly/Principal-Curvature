...
torus += np.random.normal(0, 0.5, torus.shape)
...
def epsilon_and_tau(point_cloud, query):
    ...
    eps_PCA = 0.01 * np.argmin(np.abs(np.array(ratio_all) - 0.6))
    return eps_PCA, tau
...
savetxt('torus_noise5.csv', v, delimiter=',')
...
savetxt('stats_torus_noise5.csv', stat, delimiter=',')
