...
torus += np.random.normal(0, 0.1, torus.shape)

def epsilon_and_tau(point_cloud, query):
    ratio_all = []
    for i in range(1, 250):
        epsilon_PCA = 0.01 * i
        nbrs = NearestNeighbors(n_neighbors=10, algorithm='ball_tree').fit(point_cloud)
    ...
    eps_PCA = 0.01 * np.argmin(np.abs(np.array(ratio_all) - 0.8))
    return eps_PCA, tau
...
def compute_curvature_adaptive(point_cloud, query_point, extrin_dim = 3, use_cross = True):
    ...
    max_cur_weight = np.sqrt(np.exp(-0.1 * np.array(tau_dist[max_indices]) ** 2 / np.sqrt(tau)))
    min_cur_weight = np.sqrt(np.exp(-0.1 * np.array(tau_dist[min_indices]) ** 2 / np.sqrt(tau)))
...
savetxt('torus_noise1.csv', v, delimiter=',')
...
savetxt('stats_torus_noise1.csv', stat, delimiter=',')
